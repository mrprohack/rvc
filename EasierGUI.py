...
if os.path.exists('/kaggle/working/'):
    ...
    if os.path.exists('/kaggle/working/Retrieval-based-Voice-Conversion-WebUI/hubert_base.pt'):
        # If the file exists, print a statement saying so
        print("File /kaggle/working/Retrieval-based-Voice-Conversion-WebUI/hubert_base.pt already exists. No need to download.")
    else:
        # If the file doesn't exist, print a statement saying it's downloading
        print("File /kaggle/working/Retrieval-based-Voice-Conversion-WebUI/hubert_base.pt does not exist. Starting download.")
        ...
        if response.status_code == 200:
            ...
            with open('/kaggle/working/Retrieval-based-Voice-Conversion-WebUI/hubert_base.pt', 'wb') as f:
                f.write(response.content)
            print("Download complete. File saved to /kaggle/working/Retrieval-based-Voice-Conversion-WebUI/hubert_base.pt.")
        else:
...
def mouth(size, face, voice, faces):
    ...
    if faces == 'None':
        ...
        if faces == 'Ben Shapiro':
            character = '/kaggle/working/wav2lip-HD/inputs/ben-shapiro-10.mp4'
        elif faces == 'Andrew Tate':
            character = '/kaggle/working/wav2lip-HD/inputs/tate-7.mp4'
    command = "python inference.py " \
            "--checkpoint_path checkpoints/wav2lip.pth " \
            f"--face {character} " \
            f"--audio {voice} " \
            "--pads 0 20 0 0 " \
            "--outfile /kaggle/working/wav2lip-HD/outputs/result.mp4 " \
            "--fps 24 " \
            f"--resize_factor {size}"
    process = subprocess.Popen(command, shell=True, cwd='/kaggle/working/wav2lip-HD/Wav2Lip-master')
    stdout, stderr = process.communicate()
    return '/kaggle/working/wav2lip-HD/outputs/result.mp4', 'Animation completed.'
